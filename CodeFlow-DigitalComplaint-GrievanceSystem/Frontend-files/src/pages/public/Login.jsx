...
const Login = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      // Call real backend — sends email + password, gets back JWT + user
      const result = await authAPI.login(formData.email, formData.password);

      if (!result.success) {
        setError(result.data?.message || 'Invalid credentials. Please check your email and password.');
        setLoading(false);
        return;
      }
      ...
      // Verify the role matches what was selected on the form
      const returnedRole = user?.role || result.data?.role;
      if (formData.role && returnedRole !== formData.role) {
        setError(`This account is registered as "${returnedRole}", not "${formData.role}".`);
        setLoading(false);
        return;
      }

      // Save to AuthContext (also saves token + user to localStorage)
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            ...
            <div>
              ...
              <div className="relative">
                ...
                <select
                  className="w-full px-4 py-3 pl-12 border-2 border-gray-200 rounded-xl text-gray-900 focus:outline-none focus:border-blue-500 focus:ring-4 focus:ring-blue-500/10 appearance-none bg-white"
                  value={formData.role}
            ...
            <div>
              ...
              <div className="relative">
                ...
                <input
                  type="text"
                  className="w-full px-4 py-3 pl-12 border-2 border-gray-200 rounded-xl text-gray-900 focus:outline-none focus:border-blue-500 focus:ring-4 focus:ring-blue-500/10"
            ...
            <div>
              ...
              <div className="relative">
                ...
                <input
                  ...
                  className="w-full px-4 py-3 pl-12 pr-12 border-2 border-gray-200 rounded-xl text-gray-900 focus:outline-none focus:border-blue-500 focus:ring-4 focus:ring-blue-500/10"
