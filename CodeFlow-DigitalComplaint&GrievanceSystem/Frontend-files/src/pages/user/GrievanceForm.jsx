...
const GrievanceForm = () => {
    ...
    const [form, setForm] = useState({
        ...
        rollNo: user?.studentId || user?.rollNo || user?.uid || '',
        uniEmail: user?.email || user?.uniEmail || '',
    ...
    useEffect(() => {
        if (form.category === 'ragging' && !isAnonymous) {
    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            if (result.success) {
                ...
                setSubmittedCode(result.data.code || result.data._id);
                setStep(3);
            } else {
                alert('Submission failed: ' + (result.data.message || 'Please try again.'));
            }
        } catch (error) {
        ...
        <div className="max-w-lg mx-auto mt-16 text-center">
            <div className="bg-white rounded-3xl p-10 shadow-xl">
                ...
                <div className="flex gap-3 justify-center">
                    ...
                    <button onClick={() => navigate('/user/list')} className="px-5 py-2.5 bg-gradient-to-r from-blue-900 to-teal-600 text-white rounded-xl font-semibold text-sm hover:shadow-lg transition-all">
                        View My Grievances <i className="fas fa-arrow-right ml-2"></i>
                    </button>
                </div>
            </div>
        </div>
        ...
        <div className="max-w-2xl mx-auto">
            <div className="bg-white rounded-2xl shadow-sm overflow-hidden">
                ...
                <div className="px-6 pb-6 flex gap-3">
                    <button onClick={() => setStep(1)} disabled={isSubmitting} className="flex-1 py-3 border-2 border-slate-200 text-slate-700 rounded-xl font-semibold hover:bg-slate-50 transition-colors">
                        <i className="fas fa-edit mr-2"></i>Edit
                    </button>
                    <button onClick={handleSubmit} disabled={isSubmitting} className="flex-1 py-3 bg-gradient-to-r from-blue-900 to-teal-600 text-white rounded-xl font-semibold hover:-translate-y-0.5 hover:shadow-lg transition-all disabled:opacity-70">
                        {isSubmitting ? <><i className="fas fa-spinner fa-spin mr-2"></i>Submitting...</> : <><i className="fas fa-paper-plane mr-2"></i>Submit Grievance</>}
                    </button>
                </div>
            </div>
        </div>
    ...
    return (
        <div className="max-w-3xl mx-auto">
            ...
            <div className="bg-white rounded-2xl shadow-sm p-5 mb-5">
                <div className="flex items-center justify-between">
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            <p className="font-bold text-slate-800 text-sm">Anonymous Mode</p>
            ...
            {/* Personal Info - ✅ ADDED text-slate-800 to all inputs */}
            {!isAnonymous && (
            ...
            {/* Incident Details - ✅ ADDED text-slate-800 */}
            <div className="bg-white rounded-2xl shadow-sm p-6 mb-5">
                <div className="flex items-center gap-3 mb-5">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div>
                        ...
                        <input type="date" value={form.incidentDate} onChange={e => setForm({ ...form, incidentDate: e.target.value })} required className="w-full p-3 border-2 border-slate-200 rounded-xl text-sm text-slate-800 focus:outline-none focus:border-blue-500" />
                    </div>
                    <div>
                        <label className="block text-sm font-semibold text-slate-700 mb-1">Incident Time</label>
                        <input type="time" value={form.incidentTime} onChange={e => setForm({ ...form, incidentTime: e.target.value })} className="w-full p-3 border-2 border-slate-200 rounded-xl text-sm text-slate-800 focus:outline-none focus:border-blue-500" />
                    </div>
                    <div>
                        <label className="block text-sm font-semibold text-slate-700 mb-1">Your Involvement <span className="text-red-500">*</span></label>
                        <select value={form.involvement} onChange={e => setForm({ ...form, involvement: e.target.value })} required className="w-full p-3 border-2 border-slate-200 rounded-xl text-sm text-slate-800 focus:outline-none focus:border-blue-500">
                    ...
                    <div>
                        ...
                        <input type="text" value={form.location} onChange={e => setForm({ ...form, location: e.target.value })} placeholder="e.g., Main Building, Room 101" className="w-full p-3 border-2 border-slate-200 rounded-xl text-sm text-slate-800 placeholder:text-slate-400 focus:outline-none focus:border-blue-500" />
                    </div>
                </div>
            </div>

            {/* Grievance Details - ✅ ADDED text-slate-800 */}
            <div className="bg-white rounded-2xl shadow-sm p-6 mb-5">
                <div className="flex items-center gap-3 mb-5">
                ...
                <div className="space-y-4">
                    <div>
                        ...
                        <select value={form.category} onChange={e => setForm({ ...form, category: e.target.value })} required className="w-full p-3 border-2 border-slate-200 rounded-xl text-sm text-slate-800 focus:outline-none focus:border-blue-500">
                    ...
                    <div>
                        ...
                        <input type="text" value={form.subject} onChange={e => setForm({ ...form, subject: e.target.value })} required placeholder="Brief subject of your grievance" className="w-full p-3 border-2 border-slate-200 rounded-xl text-sm text-slate-800 placeholder:text-slate-400 focus:outline-none focus:border-blue-500" />
                    </div>
                    <div>
                        <label className="block text-sm font-semibold text-slate-700 mb-1">Description <span className="text-red-500">*</span></label>
                        <textarea value={form.description} onChange={e => setForm({ ...form, description: e.target.value })} required minLength={50} rows={5} placeholder="Please provide a comprehensive description of your grievance..." className="w-full p-3 border-2 border-slate-200 rounded-xl text-sm text-slate-800 placeholder:text-slate-400 focus:outline-none focus:border-blue-500 resize-none" />
                    ...
                    {witnesses === 'yes' && (
                        <div>
                            ...
                            <textarea value={form.witnessInfo} onChange={e => setForm({ ...form, witnessInfo: e.target.value })} rows={3} placeholder="Names and contact details of witnesses" className="w-full p-3 border-2 border-slate-200 rounded-xl text-sm text-slate-800 placeholder:text-slate-400 focus:outline-none focus:border-blue-500 resize-none" />
                        </div>
                    )}
                    <div>
                        ...
                        <textarea value={form.resolutionNotes} onChange={e => setForm({ ...form, resolutionNotes: e.target.value })} required rows={3} placeholder="What outcome would resolve this grievance?" className="w-full p-3 border-2 border-slate-200 rounded-xl text-sm text-slate-800 placeholder:text-slate-400 focus:outline-none focus:border-blue-500 resize-none" />
                    </div>
                </div>
                ...
                {isAnonymous && (
                    <div className="mt-5 bg-slate-50 border-2 border-dashed border-slate-200 rounded-xl p-5 text-center">
                        ...
                        <input type="number" value={captcha.userAns} onChange={e => setCaptcha({ ...captcha, userAns: e.target.value })} placeholder="Your answer" className="w-32 p-2 border-2 border-slate-200 rounded-lg text-center text-sm text-slate-800 placeholder:text-slate-400 focus:outline-none focus:border-blue-500" />
                    </div>
                )}
            </div>
