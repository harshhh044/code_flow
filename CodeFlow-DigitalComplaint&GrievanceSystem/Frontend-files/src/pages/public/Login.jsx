...
const Login = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      // Use authService for proper validation with Backend
      ...
      if (!result.success) {
        ...
        // Show email suggestion if the error is about email not found
        const typo = detectEmailTypo(formData.email);
        if (typo.hasTypo) setEmailSuggestion(typo.suggestion);
        setLoading(false);
        return;
      }

      // Use AuthContext login — stores currentUser + token
            ...
            <div>
              ...
              {emailSuggestion && (
                <p className="text-sm text-amber-600 mt-1.5 flex items-center gap-1">
                  ...
                  Did you mean{' '}
                  <button type="button" onClick={() => { setFormData({ ...formData, email: emailSuggestion }); setEmailSuggestion(''); }} className="font-bold text-blue-700 underline hover:text-blue-900">
                    {emailSuggestion}
                  </button>?
                </p>
              )}
